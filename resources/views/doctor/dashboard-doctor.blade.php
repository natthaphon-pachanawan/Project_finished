...
<body>
        ...
        <table id="doctorTable" class="table align-items-center mb-0">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#adlModal-{{ $elderly->ID_Elderly }}">ข้อมูล ADL</button>
                        <button class="btn btn-secondary btn-sm" data-bs-toggle="modal" data-bs-target="#cgDatesModal-{{ $elderly->ID_Elderly }}">ข้อมูล CG</button>
                        <button class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#acgDatesModal-{{ $elderly->ID_Elderly }}">ข้อมูล ACG</button>
                    </td>
                </tr>
                @endforeach
            </tbody>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="adlModalLabel-{{ $elderly->ID_Elderly }}">ข้อมูล ADL ของ {{ $elderly->Name_Elderly }}</h5>
                ...
                <div class="modal-body">
                    ...
                    <table class="table table-bordered">
                        <tr>
                            ...
                            <td>{{ \App\Models\BarthelAdl::getAdlDescription('feeding', $elderly->barthel_adl->Feeding) }}</td>
                        </tr>
                        <tr>
                            <th>การดูแลร่างกาย</th>
                            <td>{{ \App\Models\BarthelAdl::getAdlDescription('grooming', $elderly->barthel_adl->Grooming) }}</td>
                        </tr>
                        <tr>
                            <th>การย้ายตัว</th>
                            <td>{{ \App\Models\BarthelAdl::getAdlDescription('transfer', $elderly->barthel_adl->Transfer) }}</td>
                        </tr>
                        <tr>
                            <th>การใช้ห้องน้ำ</th>
                            <td>{{ \App\Models\BarthelAdl::getAdlDescription('toilet_use', $elderly->barthel_adl->Toilet_use) }}</td>
                        </tr>
                        <tr>
                            <th>การเคลื่อนที่ภายในห้องหรือบ้าน</th>
                            <td>{{ \App\Models\BarthelAdl::getAdlDescription('mobility', $elderly->barthel_adl->Mobility) }}</td>
                        </tr>
                        <tr>
                            <th>การสวมใส่เสื้อผ้า</th>
                            <td>{{ \App\Models\BarthelAdl::getAdlDescription('dressing', $elderly->barthel_adl->Dressing) }}</td>
                        </tr>
                        <tr>
                            <th>การขึ้นลงบันได 1 ชั้น</th>
                            <td>{{ \App\Models\BarthelAdl::getAdlDescription('stairs', $elderly->barthel_adl->Stairs) }}</td>
                        </tr>
                        <tr>
                            <th>การอาบน้ำ</th>
                            <td>{{ \App\Models\BarthelAdl::getAdlDescription('bathing', $elderly->barthel_adl->Bathing) }}</td>
                        </tr>
                        <tr>
                            <th>การกลั้นการถ่ายอุจจาระในระยะ 1 สัปดาห์ที่ผ่านมา</th>
                            <td>{{ \App\Models\BarthelAdl::getAdlDescription('bowels', $elderly->barthel_adl->Bowels) }}</td>
                        </tr>
                        <tr>
                            <th>การกลั้นปัสสาวะในระยะ 1 สัปดาห์ที่ผ่านมา</th>
                            <td>{{ \App\Models\BarthelAdl::getAdlDescription('bladder', $elderly->barthel_adl->Bladder) }}</td>
                        </tr>
                    </table>
                    @else
                    <p>ไม่พบข้อมูล ADL</p>
    ...
    <div class="modal fade" id="cgDatesModal-{{ $elderly->ID_Elderly }}" tabindex="-1" aria-labelledby="cgDatesModalLabel-{{ $elderly->ID_Elderly }}" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="cgDatesModalLabel-{{ $elderly->ID_Elderly }}">เลือกวันที่สำหรับ CG ของ {{ $elderly->Name_Elderly }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    @php
    ...
    <div class="modal fade" id="cgDetailsModal-{{ $caregiver->ID_CG }}" tabindex="-1" aria-labelledby="cgDetailsModalLabel-{{ $caregiver->ID_CG }}" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="cgDetailsModalLabel-{{ $caregiver->ID_CG }}">ข้อมูล CG ของ {{ $elderly->Name_Elderly }} ({{ $caregiver->Date_CG }})</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <table class="table table-bordered">
    ...
    <div class="modal fade" id="acgDatesModal-{{ $elderly->ID_Elderly }}" tabindex="-1" aria-labelledby="acgDatesModalLabel-{{ $elderly->ID_Elderly }}" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="acgDatesModalLabel-{{ $elderly->ID_Elderly }}">เลือกวันที่สำหรับ ACG ของ {{ $elderly->Name_Elderly }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    @php
    ...
    <div class="modal fade" id="cgForACGModal-{{ $caregiver->ID_CG }}" tabindex="-1" aria-labelledby="cgForACGModalLabel-{{ $caregiver->ID_CG }}" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="cgForACGModalLabel-{{ $caregiver->ID_CG }}">เลือกวันที่สำหรับ ACG ของ {{ $elderly->Name_Elderly }} ({{ $caregiver->Date_CG }})</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    @php
    ...
    <div class="modal fade" id="acgDetailsModal-{{ $activity->ID_ACG }}" tabindex="-1" aria-labelledby="acgDetailsModalLabel-{{ $activity->ID_ACG }}" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="acgDetailsModalLabel-{{ $activity->ID_ACG }}">ข้อมูล ACG ของ {{ $elderly->Name_Elderly }} ({{ $activity->Date_ACG }})</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <table class="table table-bordered">
